...
class Seq2SeqModel:
    ...
    def fit_using_data_generator(self, train_generator, checkpoints_saving_path=None, save_freq=None,epochs=100, validation_generator=None, callbacks=[]):
        if checkpoints_saving_path is not None and os.path.isdir(checkpoints_saving_path):
            # Save checkpoints every epoch
            checkpoint_filepath = checkpoints_saving_path+'/checkpoint-{epoch:02d}.model.keras'
...
class TextSummaryWordLevelDataGenerator(tf.keras.utils.Sequence):
    ...
    def __prepare_data(self, df):
        ...
        target_sequences   =   pad_sequences(self.summary_tokenizer.texts_to_sequences(target_texts, sos_token=True, eos_token=True), maxlen=self.max_summary_len, padding='post')
...
class CharacterLevelTokenizer:
    def __init__(self, sos_token="<SOS>", eos_token="<EOS>", pad_token="<PAD>", oov_token="<OOV>"):
        ...
        self.special_tokens=[pad_token, sos_token, eos_token, oov_token]
        self.number_of_tokens = 0
    
    def fit_on_texts(self, texts):
        characters = self.special_tokens + sorted(set(list(" ".join(texts))))
        self.token_index = {char: i for i, char in enumerate(characters)}
        self.index_token = {i: char for i, char in enumerate(characters)}
        self.number_of_tokens = len(self.token_index)
    
    def texts_to_sequences(self, texts, sos_token=False, eos_token=False):
        encoded_data = []
        for text in texts:
            e=[]
            if sos_token:
                e.append(self.token_index[self.sos_token])
            i=0
            while i < len(text):
                if text[i:i+len(self.sos_token)] == self.sos_token:
                    e.append(self.token_index[self.sos_token])
                    i+=len(self.sos_token)
                elif text[i:i+len(self.eos_token)] == self.eos_token:
                    e.append(self.token_index[self.eos_token])
                    i+=len(self.eos_token)
                elif text[i:i+len(self.pad_token)] == self.pad_token:
                    e.append(self.token_index[self.pad_token])
                    i+=len(self.pad_token)
                elif text[i:i+len(self.oov_token)] == self.oov_token:
                    e.append(self.token_index[self.oov_token])
                    i+=len(self.oov_token)
                else:
                    e.append(self.token_index.get(text[i], self.token_index[self.oov_token]))
                    i+=1
                if eos_token:
                    e.append(self.token_index[self.eos_token])
    ...
    def sequences_to_texts(self, sequences, skip_special_characters = True):
        decoded_data = []
        for seq in sequences:
            text = ""
            for t in seq:
                if skip_special_characters:
                    text+= self.index_token[t] if self.index_token[t] not in self.special_tokens else ''
                else:
                    text+=self.index_token[t]
    ...
    def from_json(cls, json_file):
        ...
        tokenizer = cls(params['sos_token'], params['eos_token'], params['pad_token'], params['oov_token'])
        tokenizer.token_index = params['token_index']
        tokenizer.index_token = params['index_token']
        tokenizer.special_tokens = params['special_tokens']
        tokenizer.number_of_tokens = params['number_of_tokens']
        return tokenizer    
